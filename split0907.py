...
def GetLabels(txt):
    ...
    for line in fh:
        ...
        if  phaseNum == 0 :
            phaseNum = phaseNum + 1
            lastPhase = phase
        elif lastPhase != phase:
            lastPhase = phase
            phaseNum = phaseNum + 1

        phaseFlag =  (phaseNum - 1 ) % 2
        ...
        both = phaseFlag + 2 * int(color)
        temp = [phase,color,both]
        print("  temp =  ")
        print(temp)
        labels.append(temp)

        content = " phase = " + str(phase) + "  phaseNum = " + str(phaseNum)  + "  color = " + str(color) + "  colorNum = " + str(colorNum) + "  both= " + str(both)
        save_path = 'G:/liudongbo/dataset/small_pic_test/'
        fileName = "log.txt"
...
def splitImg(pic_path,pic_num,suffix,ThreshVal,SetExceptAreaVal,SetM,px,save_path,SumThreshVal,txtName,test_size,imgType):
        ...
        for m in range(len(Position) - 1,-1,-1):
           ...
           # areaVal,Position[m][0], Position[m][1], Position[m][2], Position[m][3] = saveImg(origineImage,Position[m][0], Position[m][1], Position[m][2], Position[m][3],smallImgNumber,img_p,px,save_path)
            ...
            if imgType == "delay":
                ...
                label = labels[smallImgNumber-1][1]
                content = str(smallImgNumber) + '_p'+ '.jpg' + '  ' + str(label)
            elif imgType == "both":
                label = labels[smallImgNumber-1][2]
                content = str(smallImgNumber) + '_p'+ '.jpg' + '  ' + str(label)
